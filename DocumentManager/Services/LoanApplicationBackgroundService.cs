...
public partial class LoanApplicationBackgroundService : BackgroundService
{
    ...
    private async Task ProcessLoanAsync(LoanAgreement loan, CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            // var docs = loan.DocumentSet?.Documents?.ToList() ?? new List<LiquidDocsData.Models.Document>();
    ...
    private async Task<string> BuildLenderNamesAsync(IEnumerable<LiquidDocsData.Models.Lender> lender, CancellationToken cancellationToken = default)
    {
